...
for label_num in range(0, labels.shape[1]):
    # make bert-inputs
    inputs, masks, _ = make_bert_inputs(path="../data/REST_test_x.csv", sentence_length=int(
        sentence_len), config=("./tokenizer" + sentence_len), attribute=attribute_list[label_num], pos_change=position_reversed)
